...
use libertas::{
    ...
    LibertasEndpointStandardStatus, LibertasMessageArgument, LibertasUser, LogLevel,
    ...
    libertas_formatted_text, libertas_get_sys_ticks, libertas_get_utc_time, libertas_log,
    libertas_notification_send, libertas_register_device_listener,
...
use libertas_macros::{
    LibertasAvroDecode, LibertasAvroEncode, LibertasExport, libertas_bitflags, libertas_chart,
    libertas_data_schema, libertas_export, libertas_permissions,
};
use libertas_matter::{
...
const VALVE_COMMAND_RETRY_SECONDS: u32 = 60;
const ACTIVITY_ARCHIVE_PAGE_SIZE: usize = 256;
...
const VALVE_SUBSCRIPTION_REFRESH_GRACE_SECONDS: u32 =
    VALVE_SUBSCRIPTION_MAX_INTERVAL_SECONDS as u32;
...
const MAX_WEATHER_ET_REPORT_RANGE_DAYS: usize = 31;
const MAX_WEATHER_ET_REPORT_RANGE_SECONDS: u64 =
    MAX_WEATHER_ET_REPORT_RANGE_DAYS as u64 * 24 * 60 * 60;
...
pub const APP_STRINGS: [(&str, &str); 23] = [
    (
    ...
    (
        ...
        "Saved recent rain and watering for {0}.",
    ),
    (
        "SPRINKLER_SITE_LOCATION_V1",
        "Saved location used for sprinkler weather at {0}.",
    ),
    ("SPRINKLER_WATERING_MODE_V1", "Saved watering mode for {0}."),
    (
    ...
    // Keep the resource labels for completion markers saved by the retired repair.
    (
        "SPRINKLER_ACTIVITY_CLEANUP_V1",
        "Watering history cleanup status for {0}.",
    ),
    (
        "SPRINKLER_ACTIVITY_CLEANUP_V2",
        "Daily failure history cleanup status for {0}.",
    ),
    (
        "SPRINKLER_WATERING_RESTART_V1",
        "Watering restart accounting for {0}.",
    ),
];
...
const WATERING_RESTART_RESOURCE: &str = APP_STRINGS[22].0;
...
/// Valve problems
/// Problems that a Matter irrigation valve can report at the same time. An
/// empty selection means the valve reported no problem.
#[libertas_bitflags]
pub struct SprinklerValveFaultFlagsV1(pub u16);

impl SprinklerValveFaultFlagsV1 {
    /// General problem
    /// The valve reported a problem without a more specific classification.
    pub const GENERAL_FAULT: Self = Self(1 << 0);
    /// Blocked
    /// The valve cannot move because it is blocked.
    pub const BLOCKED: Self = Self(1 << 1);
    /// Leak detected
    /// The valve detected an unexpected water leak.
    pub const LEAKING: Self = Self(1 << 2);
    /// Not connected
    /// The controller cannot detect a connected valve.
    pub const NOT_CONNECTED: Self = Self(1 << 3);
    /// Short circuit
    /// The valve or its wiring has a short circuit.
    pub const SHORT_CIRCUIT: Self = Self(1 << 4);
    /// Too much current
    /// The valve exceeded the available electrical current.
    pub const CURRENT_EXCEEDED: Self = Self(1 << 5);
}
...
pub struct SprinklerZoneActiveStateV1 {
    ...
    #[libertas_physical_unit("millimeter-per-day")]
    ...
    /// Valve problems
    /// Problems reported by the valve. More than one problem can be present.
    #[libertas_bitflags(SprinklerValveFaultFlagsV1)]
...
#[derive(Clone, Debug, PartialEq, LibertasAvroDecode, LibertasAvroEncode, LibertasExport)]
pub enum SprinklerZoneProtocol {
    /// View status
...
pub enum SprinklerWateringReasonV1 {
    ...
    LegacyUnknown,
    /// Stopped early
    /// The valve was observed closed before its timed run was due to finish.
    StoppedEarly,
}
...
pub struct SprinklerWateringActivityV1 {
    ...
    /// Actual duration
    /// Accounted valve-open duration in seconds, including reconciled brief
    /// restart gaps.
    #[libertas_time_interval]
...
/// Stores the latest complete lifecycle snapshot before any filtered archive
/// update. Plans and pending attempts stay here; observed runs and representative
/// terminal decisions also enter the archive. The current flag controls restart
/// reconciliation.
#[derive(Clone, Debug, PartialEq, LibertasAvroDecode, LibertasAvroEncode, LibertasExport)]
...
/// One recorded or planned contribution to the shared rain or one area's
/// irrigation total.
#[derive(Clone, Debug, PartialEq, LibertasAvroDecode, LibertasAvroEncode, LibertasExport)]
pub struct SprinklerWaterUsageSummaryRowV1 {
    /// Water amount
    /// Millimeter scale for comparing rain and irrigation.
    #[libertas_chart_channel(x)]
    ...
    /// Amount
    /// Total rain or irrigation represented by this part of the bar.
    #[libertas_chart_channel(tooltip)]
    #[libertas_format("0.##")]
    #[libertas_number(min = 0)]
    #[libertas_physical_unit("millimeter")]
    ...
    pub input_type: SprinklerWaterInputTypeV1,
    /// Lane
    /// Keeps shared rain first, followed by every configured area.
    #[libertas_chart_channel(y)]
    #[libertas_chart_scale(kind = band)]
    pub lane_index: u8,
    /// Rain or area
    /// Shows Rain or the configured area's name on the summary axis.
    #[libertas_formatted_text]
    #[libertas_chart_channel(tooltip)]
    #[libertas_chart_guide(
        target = y,
    ...
    pub lane_label: Vec<u8>,
}

/// Water-use summary
/// Shows shared rain and irrigation for every area together on one chart.
// Keep shared rain and every configured zone in this single unit chart; do not
// split them into separate chart or composition children in future revisions.
#[libertas_chart(bar)]
...
pub struct SprinklerWeatherEtChartV1 {
    // Each vertical panel owns a bottom UTC x-axis. The humidity layer shares
    // the middle panel's temperature axis and must not add a second one.
    /// Estimated water loss
    /// Recorded and forecast water loss through evaporation and plant use.
...
pub struct SprinklerReportRequestRangeV1 {
    /// First date
    /// Optional. Initially uses the earliest date in the chart's current
    /// maximum-size range.
    #[libertas_date_only]
    #[libertas_copy_from("$.available_range.starts_on")]
    #[libertas_range_min("{$.available_range.starts_on}")]
    #[libertas_range_max("{$.available_range.ends_on}")]
    pub starts_on: Option<u32>,
    /// Last date
    /// Optional. Initially uses the latest current date supported by the
    /// chart, including its forecast horizon when applicable.
    #[libertas_date_only]
    #[libertas_copy_from("$.available_range.ends_on")]
    #[libertas_range_min("{$.available_range.starts_on}")]
    #[libertas_range_max("{$.available_range.ends_on}")]
    pub ends_on: Option<u32>,
}

/// Available report range
/// The current inclusive date limits for one kind of sprinkler chart. The
/// range is no longer than that chart can return in one response.
#[derive(
    Clone, Copy, Debug, PartialEq, Eq, LibertasAvroDecode, LibertasAvroEncode, LibertasExport,
)]
pub struct SprinklerReportAvailableRangeV1 {
    /// Earliest date
    /// Earliest currently selectable date after applying this chart's maximum
    /// range length.
    #[libertas_date_only]
    pub starts_on: u32,
    /// Latest date
    /// Latest currently selectable date. This includes forecast dates for
    /// charts that support forecasts.
    #[libertas_date_only]
    pub ends_on: u32,
}
...
pub enum SprinklerReportProtocol {
    ...
    WaterBalanceV1 {
        ...
        /// Available range
        /// Current date-picker limits and defaults for the next request.
        #[libertas_hidden]
        available_range: SprinklerReportAvailableRangeV1,
    },
    /// View water use
    /// Shows daily rain and watering for every area. Leave the dates blank for
    /// the full current range of up to two years, ending with the supported
    /// forecast horizon. A longer selection keeps its latest two years.
    #[libertas_request]
    #[libertas_access_privilege("Read")]
...
/// Stores one independently indexed weather period or accounted irrigation
/// interval, including a recoverable timed-run projection during shutdown.
#[derive(Clone, Debug, PartialEq, LibertasAvroDecode, LibertasAvroEncode, LibertasExport)]
pub enum SprinklerWaterEventV1 {
    ...
    IrrigationV1 {
        ...
        starts_at: LibertasDateTime,
        /// Duration
        /// The accounted valve-open interval length in seconds. During shutdown
        /// it can include the remaining timed run, reconciled after restart.
        #[libertas_time_interval]
        duration_seconds: u32,
        /// Water amount adjuster
...
/// Restart accounting checkpoint
/// Preserves an observed run and its remaining timed interval across an app
/// shutdown. A valve report reconciles the interval after restart.
#[derive(Clone, Debug, PartialEq, LibertasAvroDecode, LibertasAvroEncode, LibertasExport)]
pub struct SprinklerRestartAccountingV1 {
    /// Activity
    /// Authoritative lifecycle snapshot at the checkpoint.
    pub activity: SprinklerWateringActivityV1,
    /// Accounted through
    /// Exclusive end of the already accounted valve-open interval.
    pub accounted_through: LibertasDateTime,
    /// Ledger segment
    /// Last irrigation segment, including any earlier merged prefix. A zero
    /// duration reserves its index before the first full second is accounted.
    pub segment: SprinklerWaterEventV1,
    /// Expected finish
    /// Expected end of an automatic timed run; manual runs have no known end.
    pub expected_ends_at: Option<LibertasDateTime>,
    /// Delivery rate
    /// Sprinkler-head delivery rate at shutdown, in millimeters per hour.
    pub delivery_millimeters_per_hour: f32,
}
...
pub enum SprinklerData {
    ...
    /// Watering activity
    /// Archives an observed watering run or one representative no-water failure
    /// or skip per UTC planned day, outcome, and reason, regardless of origin.
    WateringActivityV1 {
        /// Activity
    ...
    // Retain this historical marker's position and fields in the stored Avro union.
    /// Watering history cleanup
    /// Records completion of the one-time duplicate-attempt cleanup for a valve.
    ActivityArchiveCleanupV1 {
        /// Completed
        /// True after the archive has been scanned and duplicate removals verified.
        completed: bool,
    },
    /// Watering restart accounting
    /// One authoritative checkpoint per valve, cleared after reconciliation
    /// has been persisted and verified.
    WateringRestartV1 {
        /// Checkpoint
        /// Present while shutdown or restart accounting needs reconciliation.
        checkpoint: Option<SprinklerRestartAccountingV1>,
    },
}
...
struct ZoneRuntime {
    ...
    valve_subscription_refresh_started_ticks: Option<u64>,
    valve_state_initializing: bool,
    last_published_state: Option<SprinklerZoneStateV1>,
    accounted_at_ticks: Option<u64>,
    accounted_at_utc: Option<LibertasDateTime>,
    pending_command: Option<PendingValveCommand>,
    command_retry_not_before_ticks: u64,
    expected_irrigation: Option<ExpectedIrrigation>,
    current_activity: Option<SprinklerWateringActivityV1>,
    restart_accounting: Option<SprinklerRestartAccountingV1>,
    ...
    fn submit(self) {
        ...
        match evidence {
            ...
            WinterizationReminderEvidence::FreezingWeather {
                ...
                let arguments = [LibertasMessageArgument::UnitFloat {
...
fn utc_civil_date(at: LibertasDateTime) -> (i64, i64, i64) {
    let days_since_epoch = i64::try_from(at / SECONDS_PER_DAY).unwrap_or(i64::MAX / 2);
...
fn utc_day_of_year(now: LibertasDateTime) -> u16 {
    let (year, month, day) = utc_civil_date(now);
...
fn zone_key(valve: LibertasDevice) -> [LibertasMessageArgument<'static>; 1] {
    [LibertasMessageArgument::Object(valve)]
}

fn system_key(weather_endpoint: LibertasEndpoint) -> [LibertasMessageArgument<'static>; 1] {
    [LibertasMessageArgument::Object(weather_endpoint)]
}

fn report_weather_archive_key(
    weather_endpoint: LibertasEndpoint,
    generation: u64,
) -> [LibertasMessageArgument<'static>; 2] {
    [
        LibertasMessageArgument::Object(weather_endpoint),
        LibertasMessageArgument::Unsigned(generation),
    ]
}
    ...
    match libertas_data_read_single(
        ...
        _ => {
            ...
            libertas_data_write_single(
                REPORT_WEATHER_ARCHIVE_STATE_RESOURCE,
                &system_key(weather_endpoint),
                &SprinklerData::ReportWeatherArchiveStateV2 { state },
            );
            state
        }
    }
...
fn persist_site_location(weather_endpoint: LibertasEndpoint, location: SprinklerWeatherLocationV1) {
    libertas_data_write_single(
        ...
        &system_key(weather_endpoint),
        &SprinklerData::SiteLocationV1 { location },
    );
}
...
fn load_site_location(weather_endpoint: LibertasEndpoint) -> Option<SprinklerWeatherLocationV1> {
    match libertas_data_read_single(SITE_LOCATION_RESOURCE, &system_key(weather_endpoint)) {
        Some(SprinklerData::SiteLocationV1 { location }) if valid_site_location(location) => {
            Some(location)
        }
        _ => None,
    }
...
fn persist_watering_mode(weather_endpoint: LibertasEndpoint, mode: SprinklerWateringModeV1) {
    libertas_data_write_single(
        ...
        &system_key(weather_endpoint),
        &SprinklerData::WateringModeV1 { mode },
    );
}
...
fn load_watering_mode(weather_endpoint: LibertasEndpoint) -> SprinklerWateringModeV1 {
    match libertas_data_read_single(WATERING_MODE_RESOURCE, &system_key(weather_endpoint)) {
        Some(SprinklerData::WateringModeV1 { mode }) => mode,
        _ => {
...
fn persist_winterization_reminder(
    ...
    libertas_data_write_single(
        ...
        &system_key(weather_endpoint),
        &SprinklerData::WinterizationReminderV1 { memory },
    );
}
...
fn load_winterization_reminder(
    ...
    match libertas_data_read_single(
        ...
        Some(SprinklerData::WinterizationReminderV1 { memory }) if memory.last_reminded_at > 0 => {
            Some(memory)
        }
        _ => None,
    }
...
fn load_zone_memory(valve: LibertasDevice, now: LibertasDateTime) -> SprinklerZoneMemoryV1 {
    match libertas_data_read_single(ZONE_DATA_RESOURCE, &zone_key(valve)) {
        Some(SprinklerData::ZoneMemoryV1 { memory }) if valid_memory(&memory) => {
    ...
    for period in periods {
        ...
        if valid_report_weather_period(period) {
            libertas_data_write_indexed(
                database.handle,
                index,
                &SprinklerData::ReportWeatherPeriodV2 { period: *period },
            );
        }
    }
}
...
fn persist_report_weather_observation(
    ...
    libertas_data_write_indexed(
        database.handle,
        index,
        &SprinklerData::ReportWeatherObservationV1 { observation },
    );
}
...
fn allocate_watering_activity_index(
    ...
    for ordinal in 0..REPORT_ACTIVITY_INDEXES_PER_ORIGIN {
        ...
        // The SDK returns the first record at or above the requested index.
        if libertas_data_read_indexed::<SprinklerData>(database.handle, index)
            .is_none_or(|record| record.index != index)
        {
...
fn activity_has_observed_watering(activity: &SprinklerWateringActivityV1) -> bool {
    // Any observed field protects a run, even if another field is incomplete.
    activity.actual_starts_at.is_some()
        || activity.actual_duration_seconds.is_some()
        || activity.applied_water_millimeters.is_some()
}

fn unwatered_activity_key(
    activity: &SprinklerWateringActivityV1,
) -> Option<(u64, SprinklerWateringOutcomeV1, SprinklerWateringReasonV1)> {
    (valid_watering_activity(activity)
        && !activity_has_observed_watering(activity)
        && matches!(
            activity.outcome,
            SprinklerWateringOutcomeV1::Failed | SprinklerWateringOutcomeV1::Skipped
        ))
    .then(|| {
        (
            utc_day_start(activity_anchor(activity).unwrap()),
            activity.outcome,
            activity.reason,
        )
    })
...
fn watering_activity_needs_archive(activity: &SprinklerWateringActivityV1) -> bool {
    activity_has_observed_watering(activity) || unwatered_activity_key(activity).is_some()
}

fn archive_watering_activity(valve: LibertasDevice, activity: &SprinklerWateringActivityV1) {
    // Plans and command attempts live in the single authoritative state record.
    // The indexed archive contains observed runs and sparse terminal decisions.
    if !watering_activity_needs_archive(activity) {
        return;
    }
    let database = libertas_data_open_indexed(WATERING_ACTIVITIES_RESOURCE, &zone_key(valve));
    let mut archived = activity.clone();
    if let Some(key) = unwatered_activity_key(activity) {
        let Some(mut next_index) = i64::try_from(key.0)
            .ok()
            .and_then(|day| day.checked_mul(REPORT_ACTIVITY_INDEXES_PER_SECOND))
        else {
            return;
        };
        // Read the day in pages to bound memory regardless of archive size.
        'pages: while database.count > 0 && next_index <= database.max_index {
            ...
            let Some(last_index) = records.last().map(|record| record.index) else {
                break;
            };
            for record in records {
                if record.index.div_euclid(REPORT_ACTIVITY_INDEXES_PER_SECOND)
                    >= i64::try_from(key.0.saturating_add(SECONDS_PER_DAY)).unwrap_or(i64::MAX)
                {
                    break 'pages;
                }
                if let SprinklerData::WateringActivityV1 { activity: saved } = record.data
                    && saved.activity_index == record.index
                    && unwatered_activity_key(&saved) == Some(key)
                {
                    // Retain the first attempt's identity and planned fields;
                    // updated_at records the latest identical terminal failure.
                    archived = saved;
                    archived.updated_at = archived.updated_at.max(activity.updated_at);
                    break 'pages;
                }
            }
            let Some(next) = last_index.checked_add(1).filter(|next| *next > next_index) else {
                break;
            };
            next_index = next;
        }
    ...
    libertas_data_write_indexed(
        database.handle,
        archived.activity_index,
        &SprinklerData::WateringActivityV1 { activity: archived },
    );
}
...
fn persist_watering_activity_state(valve: LibertasDevice, state: SprinklerWateringActivityStateV2) {
    libertas_data_write_single(
        ...
        &zone_key(valve),
        &SprinklerData::WateringActivityStateV2 { state },
    );
}
...
fn persist_watering_activity(valve: LibertasDevice, activity: &SprinklerWateringActivityV1) {
    ...
    archive_watering_activity(valve, activity);
}
...
fn load_current_watering_activity(valve: LibertasDevice) -> Option<SprinklerWateringActivityV1> {
    ...
    for record in records {
        if let SprinklerData::WateringActivityV1 { activity } = record.data
...
fn persist_daily_reports(valve: LibertasDevice, reports: &[SprinklerDailyReportV1]) {
    ...
    for report in reports {
        ...
        libertas_data_write_indexed(
            database.handle,
            index,
            &SprinklerData::DailyReportV1 { report: *report },
        );
        assert!(
            matches!(
                libertas_data_read_indexed::<SprinklerData>(database.handle, index),
                Some(IndexedData { index: saved_index, data: SprinklerData::DailyReportV1 { report: saved } })
                    if saved_index == index && saved == *report
            ),
            "Daily checkpoint write was not confirmed; ledger must be retained"
        );
    }
}
...
fn load_modeled_weather_gaps(
    ...
    for record in records {
        ...
        if let SprinklerData::ModeledWeatherGapV1 { gap } = record.data
...
fn persist_modeled_gap_delta(
    ...
    for gap in current {
        ...
        libertas_data_write_indexed(
            database.handle,
            index,
            &SprinklerData::ModeledWeatherGapV1 { gap: *gap },
        );
    }
    for gap in previous {
...
fn load_report_weather_observations(
    ...
    for record in records {
        ...
        if let SprinklerData::ReportWeatherObservationV1 { observation } = record.data
...
fn merge_report_activity(
    ...
    record: IndexedData<SprinklerData>,
    range: SprinklerReportTimeRangeV1,
    maximum: usize,
) -> Result<(), ()> {
    let SprinklerData::WateringActivityV1 { activity } = record.data else {
        return Ok(());
    };
...
fn persist_zone_runtime_change(
    ...
    if memory != previous_memory {
        ...
        assert!(
            matches!(
                libertas_data_read_single::<SprinklerData>(ZONE_DATA_RESOURCE, &zone_key(valve)),
                Some(SprinklerData::ZoneMemoryV1 { memory: saved }) if saved == *memory
            ),
            "Balance checkpoint write was not confirmed; ledger must be retained"
        );
    }
...
fn valid_date_only(date: u32) -> bool {
    let year = date / 10_000;
    let month = date / 100 % 100;
    let day = date % 100;
    (1000..=9999).contains(&year)
        && day > 0
        && gregorian_month_days(year, month).is_some_and(|month_days| day <= month_days)
}

fn date_only_to_utc_day_start(date: u32) -> Option<LibertasDateTime> {
    let year = date / 10_000;
    let month = date / 100 % 100;
    let day = date % 100;
    // LibertasDateTime cannot represent a pre-epoch instant. DateOnly itself
    // accepts four-digit Gregorian years, so enforce the narrower report range
    // here as well as validating messages at the platform boundary.
    if year < 1970 || !valid_date_only(date) {
        return None;
    }
...
fn utc_date_only(at: LibertasDateTime) -> Option<u32> {
    let (year, month, day) = utc_civil_date(at);
...
fn calculate_active_state(
    ...
    condition = if zone.valve_fault_bitmap != 0 {
        ...
        if zone.valve_state_initializing {
...
fn valve_permits_automatic_watering(
    ...
    watering_mode == SprinklerWateringModeV1::Active
        ...
        && zone.valve_subscription_refresh_started_ticks.is_none()
...
fn take_changed_public_zone_state(
    zone: &mut ZoneRuntime,
    watering_mode: SprinklerWateringModeV1,
) -> Option<SprinklerZoneStateV1> {
    // Endpoint subscription data is a change stream. Internal recalculation
    // fields such as calculated_at must never cause an unchanged public StateV1
    // snapshot to be sent again.
    let state = public_zone_state(zone, watering_mode);
    if zone.last_published_state.as_ref() == Some(&state) {
        return None;
    }
    zone.last_published_state = Some(state.clone());
    Some(state)
}
...
fn defer_valve_command_retry(zone: &mut ZoneRuntime, now_ticks: u64) {
    zone.command_retry_not_before_ticks =
        zone.command_retry_not_before_ticks
            .max(absolute_interval_ticks(
                now_ticks,
                VALVE_COMMAND_RETRY_SECONDS,
            ));
}

fn valve_command_retry_ready(zone: &ZoneRuntime, now_ticks: u64) -> bool {
    now_ticks >= zone.command_retry_not_before_ticks
}

fn valve_actions_paused(state: &ControllerState) -> bool {
    state.shutting_down
        || state
            .zones
            .iter()
            .any(|zone| zone.restart_accounting.is_some())
}
...
fn evaluate_controller(shared: &Rc<RefCell<ControllerState>>) -> EvaluationOutcome {
        ...
        if let Some(timed_out) = zone.pending_command.filter(|pending| {
            ...
            match timed_out.kind {
                ...
                ValveCommandKind::Close => {
                    defer_valve_command_retry(zone, now_ticks);
                    if let Some(activity) = zone.current_activity.as_mut()
                        && activity.outcome == SprinklerWateringOutcomeV1::Running
                    {
                        activity.reason = SprinklerWateringReasonV1::CommandTimeout;
    ...
    let action = if valve_actions_paused(&state) {
        None
    } else if any_open {
        state
            .zones
            ...
            .enumerate()
            .find(|(_, zone)| {
                valve_command_retry_ready(zone, now_ticks)
...
fn publish_zone_state(shared: &Rc<RefCell<ControllerState>>, zone_index: usize) {
    let (endpoint, message) = {
        ...
        let Some(public_state) = take_changed_public_zone_state(zone, watering_mode) else {
            return;
        };
        (
    ...
    match MatterDevice::new(valve).invoke(&Close {}) {
        ...
        Err(error) => {
            let failed_activity = {
                ...
                zone.pending_command = None;
                defer_valve_command_retry(zone, libertas_get_sys_ticks());
...
fn account_all_open_valves(shared: &Rc<RefCell<ControllerState>>) {
        ...
        for (zone_index, zone) in state.zones.iter_mut().enumerate() {
            ...
            if account_open_zone(zone, now_ticks, now_utc, site_location) {
                changed.push((
                    ...
                    watering_recovery::capture(zone),
    ...
    ) in &changed
    {
        if let Some(checkpoint) = checkpoint {
            watering_recovery::write(*valve, Some(checkpoint.clone()));
        }
        ...
        if let Some(checkpoint) = checkpoint {
            watering_recovery::persist(*valve, checkpoint);
        }
    }
    if !changed.is_empty() {
...
fn set_valve_open_state(shared: &Rc<RefCell<ControllerState>>, zone_index: usize, is_open: bool) {
    ...
    let (
        ...
        let previous_memory = zone.memory.clone();
        let previous_events = zone.water_events.clone();
        let mut checkpoint = if let Some(saved) = &zone.restart_accounting {
            let Some(now) = now_utc else {
                return;
            };
            let confirmed = watering_recovery::confirmed(saved, is_open, now)
                .expect("Cannot reconcile watering across an invalid clock or interval");
            watering_recovery::replace_segment(&mut zone.water_events, &confirmed.segment);
            zone.current_activity = Some(confirmed.activity.clone());
            zone.restart_accounting = None;
            Some(confirmed)
        } else {
            None
        };
        zone.valve_state_known = true;
        zone.valve_state_initializing = false;
        zone.valve_last_report_ticks = Some(now_ticks);
        zone.valve_subscription_refresh_started_ticks = None;
        let mut irrigation_changed = checkpoint.is_some();
        let mut activities_to_persist = Vec::new();
        if was_open && !is_open {
            irrigation_changed = account_open_zone(zone, now_ticks, now_utc, site_location);
            checkpoint = watering_recovery::capture(zone);
        ...
        if was_open != is_open {
            ...
            zone.pending_command = None;
            if is_open {
                // An observed open resolves the previous failed-open condition;
                // an unsafe-weather close must be eligible immediately.
                zone.command_retry_not_before_ticks = 0;
        ...
        if let Some(checkpoint) = &mut checkpoint {
            checkpoint.activity = zone.current_activity.clone().unwrap();
            if !is_open
                && checkpoint
                    .expected_ends_at
                    .is_some_and(|end| now_utc.is_some_and(|now| now < end))
            {
                watering_recovery::mark_early_stop(&mut checkpoint.activity);
                zone.current_activity = Some(checkpoint.activity.clone());
                activities_to_persist.clear();
                activities_to_persist.push(checkpoint.activity.clone());
            }
        }
        (
            irrigation_changed
...
fn handle_valve_command_response(
    ...
    let result = match pending.kind {
        ValveCommandKind::Open => decode_command_response::<Open>(data),
        ValveCommandKind::Close => decode_command_response::<Close>(data),
    };
    if !matches!(result, Ok(MatterResponse::Status(0))) {
        ...
        libertas_log(
            LogLevel::Warn,
            &alloc::format!("Matter Valve {:?} command failed: {result:?}", pending.kind),
        );
        evaluate_and_publish(shared);
    }
}
...
fn handle_valve_event(
    ...
    if context.shared.borrow().shutting_down {
        return;
    }
    ...
    let valves: Vec<_> = {
        ...
        for zone in &mut state.zones {
            ...
            if (!zone.valve_state_known || stale)
                && zone.valve_subscription_refresh_started_ticks.is_none()
            {
                zone.valve_subscription_refresh_started_ticks = Some(now_ticks);
            }
        }
        state
            .zones
...
fn update_valve_subscription_health(zones: &mut [ZoneRuntime], now_ticks: u64) -> (bool, bool) {
    let stale_interval =
        u64::from(VALVE_SUBSCRIPTION_STALE_SECONDS).saturating_mul(MICROSECONDS_PER_SECOND);
    let refresh_grace =
        u64::from(VALVE_SUBSCRIPTION_REFRESH_GRACE_SECONDS).saturating_mul(MICROSECONDS_PER_SECOND);
    let mut subscription_needed = false;
    let mut schedule_changed = false;
    for zone in zones {
        let refresh_timed_out = zone
            .valve_subscription_refresh_started_ticks
            .is_some_and(|started_at| now_ticks.saturating_sub(started_at) >= refresh_grace);
...
fn refresh_valve_subscriptions(shared: &Rc<RefCell<ControllerState>>, now_ticks: u64) {
    let (subscription_needed, schedule_changed) = {
        let mut state = shared.borrow_mut();
        update_valve_subscription_health(&mut state.zones, now_ticks)
    };
...
fn handle_site_location_event(
    ...
    let shared = context
        ...
        .unwrap();
    if shared.borrow().shutting_down {
        return LibertasEndpointHandlerResult::Status(LibertasEndpointStandardStatus::Unavailable);
    }
...
fn water_balance_report_anchor(
    zone: &ReportZoneData,
    range: SprinklerReportTimeRangeV1,
) -> Option<(LibertasDateTime, LibertasDateTime, f32, f32)> {
    ...
    containing_anchor
        ...
        .or_else(|| {
            preceding_anchor.map(|anchor| {
                // Replay from the checkpoint's actual closing instant. Moving
                // its value directly to the requested boundary would discard
                // intervening rain, ET, and irrigation.
                (
                    anchor.coverage_ends_before,
                    range.starts_at,
...
fn water_balance_archive_starts_at(
    zones: &[ReportZoneData],
    range: SprinklerReportTimeRangeV1,
) -> LibertasDateTime {
    zones
        .iter()
        .filter_map(|zone| water_balance_report_anchor(zone, range))
        .map(|(replay_starts_at, _, _, _)| replay_starts_at)
        .fold(utc_day_start(range.starts_at), LibertasDateTime::min)
}
...
fn water_balance_points(
    ...
    let report_anchor = water_balance_report_anchor(zone, range);
    let active_anchor = || {
...
fn water_usage_display_full_scale_seconds() -> u64 {
    WATER_USAGE_DISPLAY_FULL_SCALE_SECONDS
}
...
fn water_usage_display_bucket_anchors(
    ...
    full_scale_seconds: u64,
) -> Vec<(LibertasDateTime, LibertasDateTime)> {
    let mut bucket_extents: Vec<(LibertasDateTime, u64)> = Vec::new();
    for total in totals {
        ...
        .collect();
        let extent =
            water_usage_display_intervals(&amounts, maximum_total_millimeters, full_scale_seconds)
                .last()
                .map(|(_, end)| *end)
                .unwrap_or_default();
        if let Some((_, maximum_extent)) = bucket_extents
            .iter_mut()
            .find(|(real_bucket_start, _)| *real_bucket_start == total.starts_at)
        {
            *maximum_extent = (*maximum_extent).max(extent);
        } else {
            bucket_extents.push((total.starts_at, extent));
        }
    }
    bucket_extents.sort_unstable_by_key(|(real_bucket_start, _)| *real_bucket_start);

    let mut display_bucket_starts = Vec::with_capacity(bucket_extents.len());
    let mut previous_bucket = None;
    let mut display_bucket_start = 0_u64;
    for (real_bucket_start, extent) in bucket_extents {
        if let Some((previous_real_bucket_start, previous_extent)) = previous_bucket {
            let real_gap_seconds = real_bucket_start.saturating_sub(previous_real_bucket_start);
            let display_gap_seconds =
                if real_gap_seconds < WATER_USAGE_PRESERVED_REAL_GAP_LIMIT_SECONDS {
                    real_gap_seconds
                } else {
                    WATER_USAGE_BROKEN_TIME_GAP_SECONDS
                };
            // Use the greatest rounded extent at this real date across every
            // zone, rather than a per-zone or nominal span, so aligned stacks
            // never overlap and empty space does not dominate their ratios.
            display_bucket_start = display_bucket_start
                .saturating_add(previous_extent)
                .saturating_add(display_gap_seconds);
        }
        display_bucket_starts.push((real_bucket_start, display_bucket_start));
        previous_bucket = Some((real_bucket_start, extent));
    }
    display_bucket_starts
}

fn water_usage_display_bucket_anchor(
    display_bucket_starts: &[(LibertasDateTime, LibertasDateTime)],
    real_bucket_start: LibertasDateTime,
) -> Option<LibertasDateTime> {
    display_bucket_starts
        .binary_search_by_key(&real_bucket_start, |(real, _)| *real)
        .ok()
        .map(|index| display_bucket_starts[index].1)
}

fn water_usage_display_offset_seconds(
    cumulative_amount_millimeters: f64,
    maximum_total_millimeters: f64,
    full_scale_seconds: u64,
) -> u64 {
    if cumulative_amount_millimeters <= 0.0
        || !cumulative_amount_millimeters.is_finite()
        || maximum_total_millimeters <= 0.0
        || !maximum_total_millimeters.is_finite()
        || full_scale_seconds == 0
    {
        return 0;
    }
    let bounded_amount = cumulative_amount_millimeters.min(maximum_total_millimeters);
    let offset = (full_scale_seconds as f64 * bounded_amount / maximum_total_millimeters) as u64;
    offset.clamp(1, full_scale_seconds)
}

fn water_usage_display_intervals(
    amounts_millimeters: &[f32],
    maximum_total_millimeters: f64,
    full_scale_seconds: u64,
) -> Vec<(u64, u64)> {
    if full_scale_seconds < amounts_millimeters.len() as u64 {
        return Vec::new();
    }
    let mut intervals = Vec::with_capacity(amounts_millimeters.len());
    let mut cumulative_amount_millimeters = 0.0_f64;
    let mut display_offset_seconds = 0_u64;
    for (index, amount_millimeters) in amounts_millimeters.iter().copied().enumerate() {
        cumulative_amount_millimeters += f64::from(amount_millimeters);
        let projected_end = water_usage_display_offset_seconds(
            cumulative_amount_millimeters,
            maximum_total_millimeters,
            full_scale_seconds,
        );
        // Project cumulative boundaries, not independently rounded widths.
        // Reserve one second for each remaining positive segment so tiny
        // contributors stay nondegenerate without allowing their rounding
        // floors to push the complete stack beyond its synthetic full scale.
        let remaining_segments = (amounts_millimeters.len() - index - 1) as u64;
        let latest_end = full_scale_seconds.saturating_sub(remaining_segments);
        let display_end_seconds = projected_end
            .max(display_offset_seconds.saturating_add(1))
            .min(latest_end);
        intervals.push((display_offset_seconds, display_end_seconds));
        display_offset_seconds = display_end_seconds;
    }
    intervals
}
...
fn accumulate_water_interval(
    ...
    while represented_at < represented_end {
        ...
        let total = if let Some(total) = totals.iter_mut().find(|total| {
            total.starts_at == bucket_start && total.ends_at == bucket_end && total.zone == zone
        }) {
            total
        } else {
...
fn water_usage_summary_rows(
    zones: &[ReportZoneData],
    totals: &[UsageAccumulator],
) -> Result<Vec<SprinklerWaterUsageSummaryRowV1>, ()> {
    ...
    if let Some(zone) = zones.first() {
        let rain_row_start = rows.len();
        let lane_label = libertas_formatted_text(WATER_USAGE_RAIN_LANE_RESOURCE, &[]);
    ...
    for (zone_index, zone) in zones.iter().enumerate() {
        let lane_index = u8::try_from(zone_index.checked_add(1).ok_or(())?).map_err(|_| ())?;
        let lane_label = libertas_formatted_text(
            WATER_USAGE_ZONE_LANE_RESOURCE,
            &[LibertasMessageArgument::Object(zone.valve)],
        );
        let zone_row_start = rows.len();
        let mut amount_starts_at_millimeters = 0.0_f64;
        for input_type in [
...
fn build_water_usage_in_time_zone(
    ...
    let full_scale_seconds = water_usage_display_full_scale_seconds();
    let display_bucket_starts =
        water_usage_display_bucket_anchors(&totals, maximum_total_millimeters, full_scale_seconds);
    let mut inputs = Vec::new();
    for total in totals {
        let Some(_) = water_usage_total_millimeters(&total) else {
            continue;
        };
        let Some(display_bucket_start) =
            water_usage_display_bucket_anchor(&display_bucket_starts, total.starts_at)
        else {
            continue;
        };
        let positive_inputs: Vec<_> = [
            (SprinklerWaterInputTypeV1::Rain, total.rain),
            (SprinklerWaterInputTypeV1::Irrigation, total.irrigation),
            (SprinklerWaterInputTypeV1::ForecastRain, total.forecast_rain),
            (
        ...
        .into_iter()
        .filter(|(_, amount)| *amount > 0.0 && amount.is_finite())
        .collect();
        let amounts: Vec<_> = positive_inputs.iter().map(|(_, amount)| *amount).collect();
        let display_intervals =
            water_usage_display_intervals(&amounts, maximum_total_millimeters, full_scale_seconds);
        for ((input_type, amount_millimeters), (display_start, display_end)) in
            positive_inputs.into_iter().zip(display_intervals)
        {
...
impl SprinklerReportChartKind {
    ...
    fn default_span_seconds(self) -> u64 {
        match self {
            ...
            Self::WaterUsage => MAX_WATER_USAGE_REPORT_RANGE_SECONDS,
    ...
    fn maximum_range_days(self) -> usize {
        match self {
            Self::WaterBalance => MAX_WATER_BALANCE_REPORT_RANGE_DAYS,
            Self::WaterUsage => MAX_WATER_USAGE_REPORT_RANGE_DAYS,
            Self::WeatherEt => MAX_WEATHER_ET_REPORT_RANGE_DAYS,
        }
    }

    fn default_range(self, now: LibertasDateTime) -> SprinklerReportTimeRangeV1 {
        let latest_day = utc_day_start(now.saturating_add(if self.includes_forecast() {
            u64::from(SPRINKLER_FORECAST_HORIZON_SECONDS)
        } else {
            0
        }));
        let starts_at = latest_day.saturating_sub(
            (self.maximum_range_days().saturating_sub(1) as u64).saturating_mul(SECONDS_PER_DAY),
        );
        SprinklerReportTimeRangeV1 {
            starts_at,
            ends_before: latest_day.saturating_add(SECONDS_PER_DAY),
        }
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct SprinklerReportDataExtent {
    first_at: LibertasDateTime,
    last_at: LibertasDateTime,
}

fn extend_report_data_extent(
    ...
    if let Some(extent) = extent {
        extent.first_at = extent.first_at.min(first_at);
        extent.last_at = extent.last_at.max(last_at);
    } else {
        *extent = Some(SprinklerReportDataExtent { first_at, last_at });
    }
}
...
fn extend_report_indexed_database_extent(
    ...
    if database.count == 0 || database.min_index > database.max_index {
        return;
    }
    let Some(first_at) = u64::try_from(database.min_index.div_euclid(indexes_per_second)).ok()
    else {
        return;
    };
    let Some(last_at) = u64::try_from(database.max_index.div_euclid(indexes_per_second)).ok()
    else {
        return;
    };
...
fn report_forecast_data_extent(
    forecast: Option<&SprinklerWeatherForecastV1>,
) -> Option<SprinklerReportDataExtent> {
    let mut extent = None;
    for period in forecast.into_iter().flat_map(|forecast| &forecast.periods) {
        let Some(ends_before) = period
            .starts_at
            .checked_add(u64::from(period.duration_seconds))
        else {
            continue;
        };
        if period.starts_at < ends_before {
            extend_report_data_extent(&mut extent, period.starts_at, ends_before - 1);
        }
    }
    extent
}
...
fn report_data_date(
    ...
    if kind == SprinklerReportChartKind::WaterUsage {
        report_local_date_only(at, time_zone)
    } else {
        utc_date_only(at)
    }
}
...
fn report_available_range(
    kind: SprinklerReportChartKind,
    data_extent: Option<SprinklerReportDataExtent>,
    now: LibertasDateTime,
    time_zone: ReportTimeZone,
) -> Option<SprinklerReportAvailableRangeV1> {
    let latest_at = if kind.includes_forecast() {
        now.checked_add(u64::from(SPRINKLER_FORECAST_HORIZON_SECONDS))?
    } else {
        now
    };
    let ends_on = report_data_date(kind, latest_at, time_zone)?;
    let current_date = report_data_date(kind, now, time_zone)?;
    let data_starts_on = data_extent
        .and_then(|extent| report_data_date(kind, extent.first_at, time_zone))
        .unwrap_or(current_date);
    let latest_day = date_only_to_utc_day_start(ends_on)?;
    let maximum_preceding_days = kind.maximum_range_days().saturating_sub(1) as u64;
    let bounded_starts_on = utc_date_only(
        latest_day.saturating_sub(maximum_preceding_days.saturating_mul(SECONDS_PER_DAY)),
    )?;
    let starts_on = data_starts_on.max(bounded_starts_on).min(ends_on);
    Some(SprinklerReportAvailableRangeV1 { starts_on, ends_on })
}

fn clamp_report_date_bounds_to_available_range(
    starts_on: Option<u32>,
    ends_on: Option<u32>,
    available_range: SprinklerReportAvailableRangeV1,
) -> Option<(Option<u32>, Option<u32>)> {
    for date in [starts_on, ends_on].into_iter().flatten() {
        if !valid_date_only(date) {
            return None;
        }
    }
    if starts_on
    ...
    if available_range.starts_on > available_range.ends_on {
        return None;
    }
    Some((
        starts_on.map(|date| date.clamp(available_range.starts_on, available_range.ends_on)),
        ends_on.map(|date| date.clamp(available_range.starts_on, available_range.ends_on)),
    ))
}
...
fn resolve_report_date_range_in_time_zone(
    ...
    let ends_before = match ends_on {
        Some(date) => Some(date_only_to_utc_day_start(date)?.checked_add(SECONDS_PER_DAY)?),
        None => None,
    };
    let range = resolve_report_range(kind, starts_at, ends_before, trusted_now)?;
    // Keep every default or explicitly selected report window representable in
    // the same DateOnly domain offered by the request UI.
    utc_date_only(range.starts_at)?;
    utc_date_only(range.ends_before.saturating_sub(1))?;
    Some(range)
}
...
fn resolve_water_usage_report_date_range(
    ...
    let range = match (starts_local, ends_local) {
        (None, None) => {
            let now = trusted_now?;
            let local_latest = utc_to_report_local_seconds(
                now.checked_add(u64::from(SPRINKLER_FORECAST_HORIZON_SECONDS))?,
                time_zone,
            )?;
            let local_latest_day = utc_day_start(local_latest);
            let local_start = local_latest_day.checked_sub(
                (MAX_WATER_USAGE_REPORT_RANGE_DAYS.saturating_sub(1) as u64)
                    .saturating_mul(SECONDS_PER_DAY),
            )?;
            SprinklerReportTimeRangeV1 {
                starts_at: report_local_to_utc_seconds(local_start, time_zone)?,
                ends_before: report_local_to_utc_seconds(
                    local_latest_day.checked_add(SECONDS_PER_DAY)?,
                    time_zone,
                )?,
            }
        }
        (Some(local_start), None) => {
            let local_end = local_start.checked_add(MAX_WATER_USAGE_REPORT_RANGE_SECONDS)?;
            SprinklerReportTimeRangeV1 {
                starts_at: report_local_to_utc_seconds(local_start, time_zone)?,
        ...
        (None, Some(local_end)) => {
            let local_start = local_end.checked_sub(MAX_WATER_USAGE_REPORT_RANGE_SECONDS)?;
            SprinklerReportTimeRangeV1 {
                starts_at: report_local_to_utc_seconds(local_start, time_zone)?,
                ends_before: report_local_to_utc_seconds(local_end, time_zone)?,
            }
...
#[derive(Clone, Copy)]
struct SprinklerReportRangeContext {
    selected: SprinklerReportTimeRangeV1,
    available: SprinklerReportAvailableRangeV1,
}

fn build_sprinkler_report_response(
    kind: SprinklerReportChartKind,
    range_context: SprinklerReportRangeContext,
    zones: &[ReportZoneData],
    ...
    let SprinklerReportRangeContext {
        selected: range,
...
fn handle_report_endpoint(
    endpoint: LibertasEndpoint,
    opcode: u8,
    message: LibertasEndpointMessage<SprinklerReportProtocol>,
    context: &mut Box<dyn Any>,
    transaction_id: u32,
    peer: u32,
) -> LibertasEndpointHandlerResult {
    ...
    let (weather_endpoint, weather_generation, valves, forecast_extent) = {
        let state = shared.borrow();
        (
            state.weather_endpoint,
            ...
            kind.includes_forecast()
                .then(|| report_forecast_data_extent(state.weather.forecast.as_ref()))
                .flatten(),
        )
    };
    let mut data_extent =
        report_database_data_extent(kind, weather_endpoint, weather_generation, &valves);
    // The selectable range always reaches the current date, even before the
    // first archive record exists, and recognizes the live forecast as part of
    // the chart's current data without depending on it for the supported end.
    extend_report_data_extent(&mut data_extent, trusted_now, trusted_now);
    if let Some(forecast_extent) = forecast_extent {
        extend_report_data_extent(
            &mut data_extent,
            forecast_extent.first_at,
            forecast_extent.last_at,
        );
    }
    let Some(available_range) =
        report_available_range(kind, data_extent, trusted_now, report_time_zone())
    else {
        return LibertasEndpointHandlerResult::Status(LibertasEndpointStandardStatus::Unavailable);
    };
    let Some((starts_on, ends_on)) =
        clamp_report_date_bounds_to_available_range(starts_on, ends_on, available_range)
    else {
        return LibertasEndpointHandlerResult::Status(
            LibertasEndpointStandardStatus::InvalidArgument,
        );
    };
    let starts_on = starts_on.unwrap_or(available_range.starts_on);
    let ends_on = ends_on.unwrap_or(available_range.ends_on);
    let Some(range) = resolve_report_date_range(kind, Some(starts_on), Some(ends_on), None) else {
        return LibertasEndpointHandlerResult::Status(
            LibertasEndpointStandardStatus::InvalidArgument,
        );
    };
    ...
    let mut archive_range = SprinklerReportTimeRangeV1 {
        ...
        let zones = state
            ...
            .map(|zone| ReportZoneData {
                ...
                daily_reports: if needs_daily_reports {
                    build_daily_reports(zone, state.site_location, trusted_now)
                } else {
                    Vec::new()
                },
            })
    ...
    if needs_daily_reports {
        ...
        // A missing day may make the closest trustworthy checkpoint older
        // than the selected range. Load every input needed to advance that
        // checkpoint, but continue to render only the requested range.
        archive_range.starts_at = water_balance_archive_starts_at(&zones, range);
    }
...
fn handle_zone_endpoint(
    endpoint: LibertasEndpoint,
    opcode: u8,
    message: LibertasEndpointMessage<SprinklerZoneProtocol>,
    context: &mut Box<dyn Any>,
    transaction_id: u32,
    peer: u32,
) -> LibertasEndpointHandlerResult {
    ...
    match message {
        ...
        SprinklerZoneProtocol::SetWateringPercentV1 { watering_percent } => {
            ...
            if zone.memory.watering_percent != watering_percent {
                ...
                let checkpoint = watering_recovery::capture(zone);
    ...
    let response = {
        ...
        match response_kind {
            ZoneResponseKind::State => SprinklerZoneProtocol::StateV1 {
    ...
    let changed = {
        ...
        for zone in &mut state.zones {
            ...
            if synchronize_history(&zone.memory, &mut zone.water_events, history.as_ref()) {
                ...
                let fold_time = watering_recovery::fold_time(zone, now);
...
fn weather_request(shared: &Rc<RefCell<ControllerState>>) -> SprinklerWeatherProtocol {
    let state = shared.borrow();
    let now = utc_seconds();
...
fn handle_weather_event(
    _endpoint: LibertasEndpoint,
    opcode: u8,
    message: LibertasEndpointMessage<SprinklerWeatherProtocol>,
    context: &mut Box<dyn Any>,
    _transaction_id: u32,
    _peer: u32,
) -> LibertasEndpointHandlerResult {
    ...
    let accepted = match (opcode, message) {
        (
            OP_ENDPOINT_RSP,
            LibertasEndpointMessage::Data(SprinklerWeatherProtocol::WeatherRecoveryV1 {
        ...
        (
            OP_ENDPOINT_DATA,
            LibertasEndpointMessage::Data(SprinklerWeatherProtocol::WeatherIncrementV1 { report }),
        ) => {
...
/// Smart Sprinkler
/// Waters each lawn, garden, or planted area according to its plants, watering
/// method, recent rain, forecast weather, and user-selected adjustments. The
/// user chooses the weather source, where area controls and reports will be
/// available, reminder recipients, and each area's valve, plant type, and
/// watering method. The user can manually change each area's watering
/// percentage and no-watering periods at any time, or winterize and reactivate
/// the whole system. The sprinkler automatically schedules and operates each
/// valve around rain, forecast precipitation, wind, freezing temperatures,
/// plant needs, and no-watering periods, and uses saved estimates during short
/// weather outages. The sprinkler shows each area's current status, water
...
pub fn libertas_sprinkler(
    ...
    // Recover the complete water ledger in short timer callbacks before enabling
    // the controller. Return from the App entry point immediately.
    ledger_recovery::start(zones, move |recovered| {
        start_sprinkler_controller(
            weather_endpoint,
            report_endpoint,
            reminder_recipients,
            recovered,
...
fn start_sprinkler_controller(
    weather_endpoint: LibertasEndpoint,
    report_endpoint: LibertasEndpoint,
    reminder_recipients: Vec<LibertasUser>,
    zones: Vec<ledger_recovery::RecoveredZone>,
) {
    ...
    for ledger_recovery::RecoveredZone {
        configuration,
        mut memory,
        mut water_events,
    } in zones
    {
        ...
        let mut restart_accounting = watering_recovery::load(configuration.valve);
        if restart_accounting.is_none()
            && let Some(activity) = load_current_watering_activity(configuration.valve)
        {
            restart_accounting = watering_recovery::previous_build_checkpoint(
                &activity,
                &water_events,
                configuration.sprinkler_head_type,
            );
        }
        if let Some(checkpoint) = &restart_accounting {
            assert!(
                memory.balance_baseline_at <= checkpoint.accounted_through,
                "Unreconciled watering was folded into the baseline"
            );
            watering_recovery::restore(configuration.valve, &mut water_events, checkpoint);
            if checkpoint.activity.outcome != SprinklerWateringOutcomeV1::Running {
                restart_accounting = None;
            }
        }
        let fold_time = restart_accounting
            .as_ref()
            .map_or(now, |saved| now.min(saved.accounted_through));
        ...
        .expect("Modeled weather recovery was incomplete; automatic watering must remain stopped");
        ...
        let runtime = ZoneRuntime {
            ...
            valve_subscription_refresh_started_ticks: None,
            valve_state_initializing: true,
            last_published_state: None,
            accounted_at_ticks: None,
            accounted_at_utc: None,
            pending_command: None,
            command_retry_not_before_ticks: 0,
            expected_irrigation,
            current_activity,
            restart_accounting,
    ...
    watering_recovery::register_shutdown(&shared);
    ...
    for zone_index in 0..zone_count {
        ...
        libertas_register_endpoint_status_listener::<SprinklerZoneProtocol, _>(
            endpoint,
    ...
    libertas_register_endpoint_status_listener::<SprinklerReportProtocol, _>(
        report_endpoint,
        handle_report_endpoint,
        Box::new(Rc::clone(&shared)),
    );
    libertas_register_endpoint_status_listener::<SprinklerWeatherProtocol, _>(
        weather_endpoint,
...
mod tests {
    ...
    pub(super) fn completed_report_activity(
        starts_at: LibertasDateTime,
    ...
    pub(super) fn runtime(memory: SprinklerZoneMemoryV1) -> ZoneRuntime {
        ...
        ZoneRuntime {
            ...
            valve_subscription_refresh_started_ticks: None,
            valve_state_initializing: false,
            last_published_state: None,
            accounted_at_ticks: None,
            accounted_at_utc: None,
            pending_command: None,
    ...
    #[test]
    fn shutdown_and_unconfirmed_restart_block_controller_actions() {
        let mut state = controller_state();
        assert!(!valve_actions_paused(&state));
        ...
        activity.outcome = SprinklerWateringOutcomeV1::Running;
        zone.restart_accounting = watering_recovery::previous_build_checkpoint(
            &activity,
            &[],
            zone.configuration.sprinkler_head_type,
        );
        assert!(zone.restart_accounting.is_some());
        state.zones.push(zone);
        assert!(valve_actions_paused(&state));
        state.zones[0].restart_accounting = None;
        assert!(!valve_actions_paused(&state));
    }

    #[test]
    fn unchanged_public_zone_state_is_not_reported_again() {
        let mut zone = runtime(memory());
        let mode = SprinklerWateringModeV1::Active;

        assert!(take_changed_public_zone_state(&mut zone, mode).is_some());
        zone.active_state.calculated_at += SCHEDULE_EVALUATION_INTERVAL_SECONDS as u64;
        assert_eq!(take_changed_public_zone_state(&mut zone, mode), None);

        zone.active_state.condition = SprinklerScheduleConditionV1::Scheduled;
        assert!(take_changed_public_zone_state(&mut zone, mode).is_some());
        assert_eq!(take_changed_public_zone_state(&mut zone, mode), None);
    }
    ...
    fn stale_valve_state_gets_a_refresh_grace_period() {
        ...
        let (subscription_needed, schedule_changed) =
            update_valve_subscription_health(core::slice::from_mut(&mut zone), stale_at);
        assert!(subscription_needed);
        assert!(!schedule_changed);
        assert!(zone.valve_state_known);

        zone.valve_subscription_refresh_started_ticks = Some(stale_at);
        let before_timeout = stale_at
            + u64::from(VALVE_SUBSCRIPTION_REFRESH_GRACE_SECONDS - 1) * MICROSECONDS_PER_SECOND;
        let (subscription_needed, schedule_changed) =
            update_valve_subscription_health(core::slice::from_mut(&mut zone), before_timeout);
        assert!(!subscription_needed);
        assert!(!schedule_changed);
        assert!(zone.valve_state_known);

        let timed_out = stale_at
            + u64::from(VALVE_SUBSCRIPTION_REFRESH_GRACE_SECONDS) * MICROSECONDS_PER_SECOND;
        let (subscription_needed, schedule_changed) =
            update_valve_subscription_health(core::slice::from_mut(&mut zone), timed_out);
        assert!(subscription_needed);
        assert!(schedule_changed);
        assert!(!zone.valve_state_known);
    }
    ...
    #[test]
    fn unknown_initial_valve_state_is_initializing_until_refresh_times_out() {
        let mut zone = runtime(memory());
        zone.valve_state_known = false;
        zone.valve_state_initializing = true;
        zone.valve_last_report_ticks = None;
        zone.valve_subscription_refresh_started_ticks = Some(0);

        let initial = calculate_active_state(
            &zone,
            &SprinklerWeatherSnapshotV2 {
                history: None,
                current: None,
        ...
        let timed_out =
            u64::from(VALVE_SUBSCRIPTION_REFRESH_GRACE_SECONDS) * MICROSECONDS_PER_SECOND;
        let (_, schedule_changed) =
            update_valve_subscription_health(core::slice::from_mut(&mut zone), timed_out);
        assert!(schedule_changed);
        assert!(!zone.valve_state_initializing);

        let unavailable = calculate_active_state(
            &zone,
            &SprinklerWeatherSnapshotV2 {
                history: None,
                current: None,
    ...
    fn public_protocol_round_trips_through_avro() {
        ...
        for value in values {
            ...
            assert_eq!(SprinklerZoneProtocol::from_avro(&encoded), Ok(value));
        }
    }
    ...
    fn report_protocol_round_trips_all_three_chart_families() {
        ...
        for custom_request in custom_requests {
            assert_eq!(
                SprinklerReportProtocol::from_avro(&custom_request.to_avro()),
                Ok(custom_request)
            );
        }
        ...
        for request in independently_optional_requests {
            assert_eq!(
                SprinklerReportProtocol::from_avro(&request.to_avro()),
                Ok(request)
            );
        }
        ...
        let available_range = SprinklerReportAvailableRangeV1 { starts_on, ends_on };
        let responses = [
        ...
        let SprinklerReportProtocol::WaterBalanceV1 {
            chart: water_balance,
            effective_range,
            available_range: returned_available_range,
        } = &responses[0]
        else {
        ...
        assert_eq!(*returned_available_range, available_range);
        ...
        for (index, response) in responses.into_iter().enumerate() {
            ...
            assert_eq!(SprinklerReportProtocol::from_avro(&encoded), Ok(response));
        }
    }
    ...
    #[test]
    fn null_report_dates_use_each_charts_current_maximum_range() {
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(SprinklerReportTimeRangeV1 {
                starts_at: utc_day_start(NOW)
                    - (MAX_WATER_BALANCE_REPORT_RANGE_DAYS as u64 - 1) * SECONDS_PER_DAY,
                ends_before: utc_day_start(NOW) + SECONDS_PER_DAY,
            })
        );
        let usage_latest_day = utc_day_start(NOW + u64::from(SPRINKLER_FORECAST_HORIZON_SECONDS));
        assert_eq!(
            resolve_report_date_range(SprinklerReportChartKind::WaterUsage, None, None, Some(NOW),),
            Some(SprinklerReportTimeRangeV1 {
                starts_at: usage_latest_day
                    - (MAX_WATER_USAGE_REPORT_RANGE_DAYS as u64 - 1) * SECONDS_PER_DAY,
                ends_before: usage_latest_day + SECONDS_PER_DAY,
            })
        );
        assert_eq!(
            resolve_report_date_range(SprinklerReportChartKind::WeatherEt, None, None, Some(NOW),),
            Some(SprinklerReportTimeRangeV1 {
                starts_at: usage_latest_day
                    - (MAX_WEATHER_ET_REPORT_RANGE_DAYS as u64 - 1) * SECONDS_PER_DAY,
                ends_before: usage_latest_day + SECONDS_PER_DAY,
            })
        );
        assert_eq!(
    ...
    #[test]
    fn report_available_ranges_use_current_chart_limits_and_data_extent() {
        let forecast_start = date_only_to_utc_day_start(20260820).unwrap();
        assert_eq!(
            report_forecast_data_extent(Some(&safe_hourly_forecast(forecast_start, 2))),
            Some(SprinklerReportDataExtent {
                first_at: forecast_start,
                last_at: forecast_start + 2 * 3_600 - 1,
            })
        );
        let old_extent = SprinklerReportDataExtent {
            first_at: date_only_to_utc_day_start(20200101).unwrap(),
            last_at: NOW,
        };
        for (kind, maximum_days) in [
            (
                SprinklerReportChartKind::WaterBalance,
                MAX_WATER_BALANCE_REPORT_RANGE_DAYS,
            ),
            (
                SprinklerReportChartKind::WaterUsage,
                MAX_WATER_USAGE_REPORT_RANGE_DAYS,
            ),
            (
                SprinklerReportChartKind::WeatherEt,
                MAX_WEATHER_ET_REPORT_RANGE_DAYS,
            ),
        ] {
            let available =
                report_available_range(kind, Some(old_extent), NOW, report_time_zone()).unwrap();
            let expected_latest_at = NOW
                + if kind.includes_forecast() {
                    u64::from(SPRINKLER_FORECAST_HORIZON_SECONDS)
                } else {
                    0
                };
            let expected_ends_on = utc_date_only(expected_latest_at).unwrap();
            let expected_starts_on = utc_date_only(
                date_only_to_utc_day_start(expected_ends_on).unwrap()
                    - (maximum_days as u64 - 1) * SECONDS_PER_DAY,
            )
            .unwrap();
            assert_eq!(
                available,
                SprinklerReportAvailableRangeV1 {
                    starts_on: expected_starts_on,
                    ends_on: expected_ends_on,
                }
            );
        }

        let recent_extent = SprinklerReportDataExtent {
            first_at: date_only_to_utc_day_start(20260810).unwrap() + 12 * 3_600,
            last_at: NOW,
        };
        let available = report_available_range(
            SprinklerReportChartKind::WaterBalance,
            Some(recent_extent),
            NOW,
            report_time_zone(),
        ...
        .unwrap();
        assert_eq!(available.starts_on, 20260810);
        assert_eq!(
            clamp_report_date_bounds_to_available_range(Some(19691231), Some(20300101), available,),
            Some((Some(available.starts_on), Some(available.ends_on)))
        );
        assert_eq!(
            clamp_report_date_bounds_to_available_range(None, None, available),
            Some((None, None))
        );
        assert_eq!(
            clamp_report_date_bounds_to_available_range(Some(20260815), None, available),
            Some((Some(20260815), None))
        );
        assert_eq!(
            clamp_report_date_bounds_to_available_range(Some(20300101), Some(20200101), available,),
            None
        );
        assert_eq!(
            clamp_report_date_bounds_to_available_range(Some(20260229), None, available),
            None
        );
    }

    #[test]
    fn date_only_converts_valid_utc_dates_without_timezone_semantics() {
        for date in [19700101, 20000229, 20261231, 99991231] {
            let starts_at = date_only_to_utc_day_start(date).unwrap();
            assert_eq!(utc_date_only(starts_at), Some(date));
            assert_eq!(utc_date_only(starts_at + SECONDS_PER_DAY - 1), Some(date));
        }
        for date in [19691231, 20230229, 20261301, 20260000] {
            assert_eq!(date_only_to_utc_day_start(date), None);
        }
        assert_eq!(
    ...
    #[test]
    fn every_report_date_range_treats_the_last_date_as_inclusive() {
        for kind in [
            SprinklerReportChartKind::WaterBalance,
            SprinklerReportChartKind::WaterUsage,
            SprinklerReportChartKind::WeatherEt,
            ...
            let range =
                resolve_report_date_range(kind, Some(20240228), Some(20240229), None).unwrap();
            assert_eq!(
                range,
                SprinklerReportTimeRangeV1 {
                    starts_at: date_only_to_utc_day_start(20240228).unwrap(),
                    ends_before: date_only_to_utc_day_start(20240301).unwrap(),
                }
            );
            assert_eq!(range.ends_before - range.starts_at, 2 * SECONDS_PER_DAY);
        }
    }
    ...
    fn every_report_chart_includes_populated_and_idle_configured_zones() {
        ...
        assert_eq!(usage.summary.len(), 3);
        assert_eq!(usage.summary[0].lane_index, 0);
        assert_eq!(usage.summary[0].amount_millimeters, 0.0);
        assert!(usage.summary.iter().any(|row| row.lane_index == 1));
        let idle_summary = usage
            .summary
            .iter()
            .find(|row| row.lane_index == 2)
            .unwrap();
    ...
    #[test]
    fn water_usage_uses_fixed_display_scale_and_positive_intervals() {
        let full_scale_seconds = water_usage_display_full_scale_seconds();
        let maximum_total_millimeters = 4.0;

        assert_eq!(full_scale_seconds, 600);
        assert_eq!(WATER_USAGE_BROKEN_TIME_GAP_SECONDS, 30);
        let equal_amount_offset =
            water_usage_display_offset_seconds(2.0, maximum_total_millimeters, full_scale_seconds);
        assert_eq!(equal_amount_offset, 300);
        assert_eq!(
            water_usage_display_offset_seconds(4.0, maximum_total_millimeters, full_scale_seconds,),
            full_scale_seconds
        );
        assert_eq!(
            water_usage_display_offset_seconds(
        ...
        let intervals = water_usage_display_intervals(
            &[4.0, f32::MIN_POSITIVE, f32::MIN_POSITIVE, f32::MIN_POSITIVE],
            4.0,
            full_scale_seconds,
        );
        assert_eq!(intervals.len(), 4);
        assert!(intervals.iter().all(|(start, end)| end > start));
        assert!(intervals.windows(2).all(|pair| pair[0].1 == pair[1].0));
        assert_eq!(intervals.last().unwrap().1, full_scale_seconds);
    }

    #[test]
    fn water_usage_broken_time_anchors_share_dates_and_apply_gap_thresholds() {
        ...
        let anchors = water_usage_display_bucket_anchors(
            &totals,
            10.0,
            water_usage_display_full_scale_seconds(),
        );
        ...
        assert_eq!(
            anchors,
            vec![(100, 0), (159, 659), (219, 1_289), (10_000, 1_919)]
        );
        assert_eq!(water_usage_display_bucket_anchor(&anchors, 100), Some(0));
        assert!(anchors.windows(2).all(|pair| pair[0].1 < pair[1].1));
    }

    #[test]
    fn water_usage_next_anchor_follows_the_actual_widest_group_extent() {
        let total = |starts_at, zone, rain| UsageAccumulator {
            starts_at,
            ends_at: starts_at + SECONDS_PER_DAY,
            starts_on: 19700101,
        ...
        let totals = [total(100, 7, 5.0), total(100, 8, 4.0), total(1_000, 7, 2.0)];
    ...
    #[test]
    fn water_usage_multizone_rows_share_anchors_and_preserve_stack_proportions() {
        let day = utc_day_start(NOW);
        let range = SprinklerReportTimeRangeV1 {
            starts_at: day,
        ...
        let report_zone = |valve, activities| {
            let mut configuration = zone();
        ...
        let mut zone_one_actual = completed_report_activity(day);
        zone_one_actual.actual_duration_seconds = Some(3_600);
        zone_one_actual.applied_water_millimeters = Some(6.0);
        let mut zone_one_scheduled = completed_report_activity(day);
        zone_one_scheduled.outcome = SprinklerWateringOutcomeV1::Scheduled;
        zone_one_scheduled.actual_starts_at = None;
        zone_one_scheduled.actual_duration_seconds = None;
        zone_one_scheduled.applied_water_millimeters = None;
        zone_one_scheduled.scheduled_duration_seconds = Some(3_600);
        zone_one_scheduled.planned_water_millimeters = Some(3.0);
        let mut zone_one_sparse = completed_report_activity(day + 20 * SECONDS_PER_DAY);
        zone_one_sparse.actual_duration_seconds = Some(3_600);
        zone_one_sparse.applied_water_millimeters = Some(3.0);
        let mut zone_two_actual = completed_report_activity(day);
        zone_two_actual.actual_duration_seconds = Some(3_600);
        zone_two_actual.applied_water_millimeters = Some(2.0);

        let rows = build_water_usage(
            &[
                report_zone(
                    7,
                    vec![zone_one_actual, zone_one_scheduled, zone_one_sparse],
                ),
                report_zone(8, vec![zone_two_actual]),
            ],
        ...
        let row = |zone, at, input_type| {
            ...
            rows.daily
                ...
                .find(|row| {
                    row.zone == zone
                        && row.bucket_starts_on == bucket_starts_on
        ...
        let rain_summary_row = |input_type| {
            rows.summary
                .iter()
                .find(|row| row.lane_index == 0 && row.input_type == input_type)
                .unwrap()
        };
        let irrigation_summary_row = |lane_index, input_type| {
            rows.summary
                .iter()
                .find(|row| row.lane_index == lane_index && row.input_type == input_type)
                .unwrap()
        };

        assert_eq!(rows.summary.len(), 5);
        assert_eq!(
            rain_summary_row(SprinklerWaterInputTypeV1::Rain).lane_label,
            libertas_formatted_text(WATER_USAGE_RAIN_LANE_RESOURCE, &[])
        );
        assert_eq!(
            irrigation_summary_row(1, SprinklerWaterInputTypeV1::Irrigation).lane_label,
            libertas_formatted_text(
                WATER_USAGE_ZONE_LANE_RESOURCE,
                &[LibertasMessageArgument::Object(7)],
            )
        );
        assert_eq!(
            (
                rain_summary_row(SprinklerWaterInputTypeV1::Rain).lane_index,
        ...
        assert_eq!(
            (
                rain_summary_row(SprinklerWaterInputTypeV1::ForecastRain).lane_index,
        ...
        assert_eq!(
            (
                irrigation_summary_row(1, SprinklerWaterInputTypeV1::Irrigation)
                    .amount_starts_at_millimeters,
                irrigation_summary_row(1, SprinklerWaterInputTypeV1::Irrigation)
                    .amount_ends_at_millimeters,
            ),
        ...
        assert_eq!(
            (
                irrigation_summary_row(1, SprinklerWaterInputTypeV1::ScheduledWater)
                    .amount_starts_at_millimeters,
                irrigation_summary_row(1, SprinklerWaterInputTypeV1::ScheduledWater)
                    .amount_ends_at_millimeters,
            ),
        ...
        assert!(rows.summary.iter().all(|row| {
            row.lane_index != 2 || row.input_type != SprinklerWaterInputTypeV1::ScheduledWater
        }));
        ...
        assert_eq!(
            (
                ...
                row(7, day, SprinklerWaterInputTypeV1::Rain).display_end
            ),
            (0.0, 100.0)
        );
        assert_eq!(
            (
                row(7, day, SprinklerWaterInputTypeV1::Irrigation).display_start,
                row(7, day, SprinklerWaterInputTypeV1::Irrigation).display_end
            ),
            (100.0, 400.0)
        );
        assert_eq!(
            (
                row(7, day, SprinklerWaterInputTypeV1::ForecastRain).display_start,
                row(7, day, SprinklerWaterInputTypeV1::ForecastRain).display_end
            ),
            (400.0, 450.0)
        );
        assert_eq!(
            (
                row(7, day, SprinklerWaterInputTypeV1::ScheduledWater).display_start,
                row(7, day, SprinklerWaterInputTypeV1::ScheduledWater).display_end
            ),
            (450.0, 600.0)
        );
        assert_eq!(
            (
                row(8, day, SprinklerWaterInputTypeV1::Rain).display_start,
                row(8, day, SprinklerWaterInputTypeV1::Rain).display_end
            ),
            (0.0, 100.0)
        );
        assert_eq!(
            (
                row(8, day, SprinklerWaterInputTypeV1::Irrigation).display_start,
                row(8, day, SprinklerWaterInputTypeV1::Irrigation).display_end
            ),
            (100.0, 200.0)
        );
        assert_eq!(
            (
                row(8, day, SprinklerWaterInputTypeV1::ForecastRain).display_start,
                row(8, day, SprinklerWaterInputTypeV1::ForecastRain).display_end
            ),
            (200.0, 250.0)
        );
    ...
    fn water_usage_uses_exact_partial_raw_inputs_and_omits_zero_segments() {
        ...
        assert_eq!(irrigation.display_start, rain.display_end);
        assert_eq!(irrigation.display_end, 600.0);
        ...
        assert_eq!(
            (
                planned.summary[0].amount_starts_at_millimeters,
                planned.summary[0].input_type,
            ),
            (0.0, SprinklerWaterInputTypeV1::ForecastRain)
        );
        assert_eq!(
            (
                planned.summary[1].amount_starts_at_millimeters,
                planned.summary[1].input_type,
            ),
        ...
        assert_eq!(scheduled_water.display_start, forecast_rain.display_end);
        assert_eq!(scheduled_water.display_end, 600.0);
        ...
        .unwrap();
        assert_eq!(empty.summary.len(), 2);
        assert_eq!(empty.summary[0].lane_index, 0);
        assert_eq!(empty.summary[0].amount_millimeters, 0.0);
        assert_eq!(empty.summary[1].lane_index, 1);
        assert_eq!(empty.summary[1].amount_millimeters, 0.0);
    ...
    #[test]
    fn pending_attempts_use_current_state_and_only_runs_or_terminal_decisions_need_archive() {
        let mut activity = completed_report_activity(NOW);
        ...
        for outcome in [
            SprinklerWateringOutcomeV1::Scheduled,
            SprinklerWateringOutcomeV1::CommandPending,
            SprinklerWateringOutcomeV1::Superseded,
        ] {
            activity.outcome = outcome;
            assert!(!watering_activity_needs_archive(&activity));
        }
        activity.outcome = SprinklerWateringOutcomeV1::Failed;
        activity.reason = SprinklerWateringReasonV1::CommandFailed;
        assert!(watering_activity_needs_archive(&activity));
        let first_key = unwatered_activity_key(&activity).unwrap();
        let mut retry = activity.clone();
        retry.scheduled_starts_at = Some(NOW + 60);
        retry.activity_index =
            watering_activity_index(NOW + 60, retry.origin, retry.activity_ordinal).unwrap();
        assert_eq!(unwatered_activity_key(&retry), Some(first_key));
        for observed_field in 0..3 {
            let mut observed = retry.clone();
            match observed_field {
                0 => observed.actual_starts_at = Some(NOW + 60),
                1 => observed.actual_duration_seconds = Some(1),
                _ => observed.applied_water_millimeters = Some(0.0),
            }
            assert!(watering_activity_needs_archive(&observed));
            assert_eq!(unwatered_activity_key(&observed), None);
        }
    }

    #[test]
    fn failure_groups_keep_distinct_days_reasons_and_outcomes_but_merge_origins() {
        let day = utc_day_start(NOW);
        let mut keys = Vec::new();
        for offset in 0..2 {
            for kind in 0..6 {
                let mut activity = completed_report_activity(day + offset * 60 + kind);
                activity.outcome = SprinklerWateringOutcomeV1::Failed;
                activity.reason = SprinklerWateringReasonV1::CommandFailed;
                activity.actual_starts_at = None;
                activity.actual_duration_seconds = None;
                activity.applied_water_millimeters = None;
                match kind {
                    1 => activity.reason = SprinklerWateringReasonV1::NoOpenObserved,
                    2 => activity.origin = SprinklerWateringOriginV1::Manual,
                    3 => activity.outcome = SprinklerWateringOutcomeV1::Skipped,
                    4 => activity.scheduled_starts_at = Some(day + SECONDS_PER_DAY + offset),
                    5 => activity.origin = SprinklerWateringOriginV1::LegacyUnknown,
                    _ => {}
                }
                activity.activity_index = watering_activity_index(
                    activity.scheduled_starts_at.unwrap(),
                    activity.origin,
                    activity.activity_ordinal,
                )
                .unwrap();
                let key = unwatered_activity_key(&activity).unwrap();
    ...
    #[test]
    fn retired_cleanup_marker_preserves_its_persistent_union_encoding() {
        for completed in [false, true] {
            let value = SprinklerData::ActivityArchiveCleanupV1 { completed };
            assert_eq!(value.to_avro(), vec![24, u8::from(completed)]);
            assert_eq!(SprinklerData::from_avro(&value.to_avro()), Ok(value));
        }
    }

    #[test]
    fn failed_command_retry_waits_for_its_boundary_and_saturates() {
        let mut zone = runtime(memory());
        assert!(valve_command_retry_ready(&zone, 0));
        defer_valve_command_retry(&mut zone, 0);
        let deadline = u64::from(VALVE_COMMAND_RETRY_SECONDS) * MICROSECONDS_PER_SECOND;
        assert!(!valve_command_retry_ready(&zone, 0));
        assert!(!valve_command_retry_ready(&zone, deadline - 1));
        assert!(valve_command_retry_ready(&zone, deadline));
        defer_valve_command_retry(&mut zone, deadline);
        assert!(!valve_command_retry_ready(&zone, deadline));
        defer_valve_command_retry(&mut zone, u64::MAX - 1);
        assert!(!valve_command_retry_ready(&zone, u64::MAX - 1));
        assert!(valve_command_retry_ready(&zone, u64::MAX));
    }
    ...
    fn eight_hour_manual_activity_is_queryable_during_its_last_hour() {
        ...
        let record = IndexedData {
            ...
            data: SprinklerData::WateringActivityV1 {
    ...
    #[test]
    fn pruning_preserves_every_unaccounted_event_from_a_busy_current_day() {
        let day = utc_day_start(NOW);
        let mut memory = default_memory(day);
        memory.balance_baseline_at = day;
        let original_memory = memory.clone();
        let mut events: Vec<_> = (0..2_049)
            .map(|index| SprinklerWaterEventV1::IrrigationV1 {
                starts_at: day + index * 10,
                duration_seconds: 5,
                watering_percent: 100,
    ...
    fn persistent_memory_round_trips_and_discriminants_are_stable() {
        ...
        let value = SprinklerData::ZoneMemoryV1 { memory: memory() };
        let encoded = value.to_avro();
        assert_eq!(encoded.first(), Some(&0));
        assert_eq!(SprinklerData::from_avro(&encoded), Ok(value));

        let value = SprinklerData::WateringModeV1 {
        ...
        let value = SprinklerData::WaterEventV1 {
        ...
        let value = SprinklerData::SiteLocationV1 {
            location: location(),
        };
        let encoded = value.to_avro();
        ...
        let value = SprinklerData::WinterizationReminderV1 {
            memory: SprinklerWinterizationReminderMemoryV1 {
                last_reminded_at: NOW,
        ...
        let value = SprinklerData::DailyReportV1 {
    ...
    #[test]
    fn water_balance_replays_inputs_after_an_older_preceding_checkpoint() {
        let day = utc_day_start(NOW);
        let range = SprinklerReportTimeRangeV1 {
            starts_at: day + 2 * SECONDS_PER_DAY + 12 * 3_600,
            ends_before: day + 3 * SECONDS_PER_DAY,
        };
        let mut preceding_report = completed_daily_report(day);
        preceding_report.closing_deficit_millimeters = preceding_report.capacity_millimeters;
        let mut rain = report_weather_period(day + SECONDS_PER_DAY);
        rain.precipitation_millimeters = 6.0;
        rain.reference_evapotranspiration_millimeters = 0.0;
        let report_zone = ReportZoneData {
            valve: zone().valve,
            plant_type: zone().plant_type,
            capacity_millimeters: root_zone_capacity_millimeters(&zone()),
            ...
            current_activity: None,
            activities: Vec::new(),
            daily_reports: vec![preceding_report],
        };

        assert_eq!(
            water_balance_archive_starts_at(core::slice::from_ref(&report_zone), range),
            preceding_report.coverage_ends_before
        );
        assert!(preceding_report.coverage_ends_before < utc_day_start(range.starts_at));

        let chart =
            build_water_balance_chart(core::slice::from_ref(&report_zone), &[rain], range).unwrap();
        let available = chart
            .balance
            .iter()
            .find(|point| {
                point.series == SprinklerWaterBalanceSeriesV1::AvailableWater
                    && point.at == range.starts_at
            })
            .unwrap();
        let expected = 6.0 / preceding_report.capacity_millimeters * 100.0;
        assert!((available.available_water_percent - expected).abs() < 0.001);
        assert!(available.available_water_percent > 0.0);
    }
    ...
    fn truncated_persistent_data_is_rejected() {
        let encoded = SprinklerData::ZoneMemoryV1 { memory: memory() }.to_avro();
        assert!(SprinklerData::from_avro(&encoded[..encoded.len() - 1]).is_err());
    }
}
